...
export function renderPhotoScene(host, project, state) {
  try {
    ...
    // Drive thumbnails for files that are not link-visible 403 for anyone but
    // the deploying account — a broken-image icon would tell the user
    // nothing. `error` doesn't bubble, but it does propagate through the
    // capture phase, so one delegated listener on `host` (never an inline
    // on*= handler) catches every <img> in the stack. `host` is a fresh node
    // on every render (projects.js replaces the whole detail view via
    // innerHTML each time), so this listener is discarded with it rather
    // than accumulating — the same non-accumulation guarantee ui.js's bind()
    // gives bubbling listeners, just reached differently for an event type
    // that doesn't bubble.
    host.addEventListener('error', onPhotoError, true);
  } catch {
...
function onPhotoError(e) {
  const img = e.target;
  if (!(img instanceof HTMLImageElement) || !img.classList.contains('ps')) return;
  const fallback = document.createElement('div');
  fallback.className = img.className + ' ps--unavailable';
